...
try {
	...
	if (!empty($_GET)) {
		...
		if (isset($_GET['branch']) && strlen($_GET['branch']) > 1 && defined('SWITCH_TO_BRANCH') && SWITCH_TO_BRANCH === true) {
			$response_string = $svn_wrapper->switchToBranch($_GET['branch']);
		}
		//--- switch to tag
		if (isset($_GET['tag']) && strlen($_GET['tag']) > 1 && defined('SWITCH_TO_TAG') && SWITCH_TO_TAG === true) {
			$response_string = $svn_wrapper->switchToTag($_GET['tag']);
		}
		//--- switch to trunk
		if (isset($_GET['trunk']) && defined('SWITCH_TO_TRUNK') && SWITCH_TO_TRUNK === true) {
			$response_string = $svn_wrapper->switchToTrunk();
		}
		//--- switch to revision
		if (isset($_GET['revision_no']) && defined('SWITCH_TO_REVISION') && SWITCH_TO_REVISION === true) {
			$response_string = $svn_wrapper->switchToRevision(is_numeric($_GET['revision_no']) ? $_GET['revision_no'] : null);
		}
        
        $response_string = explode("\n", $response_string);
		$view->response_string = array_filter($response_string);
...
//--- populate info data
$svn_info = explode("\n", $svn_wrapper->getSvnInfo());
$svn_top_info = array();
$svn_more_info = array();
$svn_top_markers = array('URL' => 'URL', 'Revision' => 'Revision', 'Last Changed Author' => 'Last changed');

$markers_keys = array_keys($svn_top_markers);
foreach ($svn_info as $info) { 
    ...
    if (in_array($info_key, $markers_keys)) {        
        $svn_top_info[$svn_top_markers[$info_key]] = $info_value;
    } else {
        $svn_more_info[$info_key] = $info_value;
    }
}

$view->svn_info = $svn_info;
$view->svn_top_info = $svn_top_info;
$view->svn_more_info = $svn_more_info;

$view->project_name = PROJECT_NAME; 
$view->environment = APPLICATION_ENVIRONMENT;

// svn current working copy 
$view->current_position = $svn_wrapper->getCurrentPosition();
...
if (defined('SWITCH_TO_BRANCH') && SWITCH_TO_BRANCH === true) {    
    $view->active_branches = $svn_wrapper->getActiveBranches();
    $view->merged_branches = $svn_wrapper->getMergedBranches();
}

//--- show/hide tags
if (defined('SWITCH_TO_TAG') && SWITCH_TO_TAG === true) {
    $view->tags = $svn_wrapper->getTags();
}
...
if (defined('USE_DB_VERSIONING') && USE_DB_VERSIONING === true) {
	...
	try {
		...
		$view->database_data_last_update = !empty($latest_data_version[CodePax_DbVersions::DATE_ADDED_ATTRIBUTE]) 
                                         ? $latest_data_version[CodePax_DbVersions::DATE_ADDED_ATTRIBUTE] : 'n/a';
